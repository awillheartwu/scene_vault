...
async function copyText(value: string): Promise<void> {
  if (navigator.clipboard?.writeText) {
    try {
      ...
      // Fall through to the selection-based local fallback.
    }
  }
...
<template>
  <Dialog :open="true" @update:open="!$event && emit('close')">
    <DialogContent
      class="debug-log-dialog"
      :show-close-button="false"
      aria-label="调试日志"
    >
      <header class="log-dialog-header">
        <div class="log-title-block">
          <span class="log-title-icon"><FileClock :size="18" /></span>
          <div>
            <DialogTitle>调试日志</DialogTitle>
            <DialogDescription>{{ statusSummary }}</DialogDescription>
          </div>
        </div>
        <button type="button" class="log-icon-button" aria-label="关闭调试日志" @click="emit('close')">
          <X :size="18" />
        </button>
      </header>
      ...
      <footer class="log-dialog-footer">
        <div class="log-maintenance-actions">
...
.log-title-block { gap: 11px; }
.log-title-block h2 { margin: 0; font-size: 17px; font-weight: 700; }
.log-title-block p { margin: 4px 0 0; color: var(--muted-foreground); font-size: 12px; }
.log-title-icon { display: grid; width: 36px; height: 36px; place-items: center; border-radius: 10px; background: color-mix(in srgb, var(--accent) 14%, var(--secondary)); color: var(--accent); }
.log-icon-button { display: grid; width: 36px; height: 36px; place-items: center; border: 1px solid transparent; border-radius: 9px; background: transparent; color: var(--muted-foreground); }
.log-icon-button:hover { border-color: var(--border); background: var(--secondary); color: var(--foreground); }
...
.log-result-meta { display: flex; justify-content: space-between; gap: 12px; padding: 8px 16px; border-bottom: 1px solid var(--border); color: var(--muted-foreground); font-size: 11px; }
.log-result-meta span:last-child { color: var(--warn); }

.log-list { min-height: 0; flex: 1; overflow: auto; background: var(--background); }
.log-row {
  display: grid;
  grid-template-columns: 132px 54px minmax(130px, 180px) minmax(240px, 1fr);
  align-items: start;
  gap: 12px;
  min-height: 42px;
  padding: 10px 16px;
...
.log-maintenance-actions { gap: 8px; }
.log-dialog-footer button { display: inline-flex; height: 36px; align-items: center; gap: 7px; padding: 0 11px; border: 1px solid var(--border); border-radius: 8px; background: var(--secondary); color: var(--foreground); font-size: 12px; font-weight: 600; }
.log-dialog-footer button:hover:not(:disabled) { border-color: color-mix(in srgb, var(--accent) 42%, var(--border)); }
.log-dialog-footer .copy-diagnostics { border-color: transparent; background: var(--accent-gradient); color: var(--accent-foreground); }
...
@media (max-width: 760px) {
  .debug-log-dialog { width: calc(100vw - 20px); height: calc(100vh - 20px); min-height: 0; }
  .log-dialog-header { padding: 14px 16px; }
  .log-privacy-note { align-items: flex-start; padding-inline: 16px; }
  ...
  .log-dialog-footer { align-items: stretch; flex-direction: column-reverse; }
  .log-maintenance-actions { display: grid; grid-template-columns: 1fr 1fr; }
  .log-dialog-footer button { justify-content: center; }
}
</style>
