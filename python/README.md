...
- 请求串行执行。YuNet 按完整检测配置和模型文件身份缓存，SFace / ArcFace 按模型文件
  身份分别缓存；配置、路径或同路径文件变化会替换对应旧实例，加载失败不会进入缓存，
  推理异常会淘汰对应实例以便下一请求重新加载。
...
SFace / ArcFace 特征的 `faceFeatureModelVersion` 会附带模型文件 SHA-256 指纹。即使
