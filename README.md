...
- **分类与角色**：人物/游戏截图/收藏/待分类、角色标记与重命名/合并/代表头像、
  人物名称/别名查询、快捷分类弹窗（全局快捷键）
- **可选 AI**：人脸检测、Face Bank 角色建议、标注图与头像生成、SFace/ArcFace 切换
- **可靠归档**：本地或 NAS、原子写入、失败自动重试退避、NAS 断线不丢数据
- **文件生命周期**：项目级原图重新定位、归档目标找回（归档目录改名或搬迁后按采集 ID
  递归找回）与缺失/替换检查；原图永不删除，归档目标可经明确确认移入回收站或从 UNC/NAS
  永久删除
- **评分清单**：为归档的人物目录自动维护 `project.json`（人物 ↔ 图片对应表），供打分
  插件直接读取；全局开关控制自动维护，关闭后仍可手动生成
