...
Rust 已通过 Toolhelp32 构造当前应用的子进程树，并按 Rust、WebView2、Python 和辅助
进程分组向前端暴露 CPU、Working Set、Peak Working Set 与 Private Bytes。采样只在
Settings 的“资源与存储”分类实际挂载且窗口可见时运行。
...
页面可见时每 2 秒采样；窗口进入后台或页面卸载后完全停止。资源页重新可见时立即恢复。
资源采样没有接入现有 500 ms capture 轮询。
...
磁盘目录仅在首次进入页面、用户手动刷新或清理完成后扫描，不在后台定时遍历。
...
- 阶段 1 的应用进程树采样已接入资源页；Python 基准的 100/500 次耐久测试仍待补充。
- 阶段 2、3 已完成第一版：数据库（含 WAL/SHM）、日志、缩略图、处理缓存、模型、字体、
  Python runtime 与 WebView2 数据分项展示。
- 阶段 4 已完成手动清理入口：缩略图、过期日志、已完成且归档文件仍存在的处理缓存。
  处理缓存清理会先把本地图片引用切换到已验证存在的归档文件，再删除 UUID 目录；清理
  结果写入 `resource.cleanup` 日志。
- `src-tauri/target` 与 `node_modules` 属于开发工作区，不进入普通用户清理入口；运行中的
  应用不会尝试删除自身构建产物。
