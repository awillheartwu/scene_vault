...
截至 2026-08-11，阶段 1、2、3 和 3.1 稳定性复核已完成。阶段 4 增加
`SCENE_VAULT_VISION_MODE=worker|oneshot` 诊断开关和可复现基准工具；常驻模式继续保留
一次性 `request` 作为自动降级路径。
...
Windows SFace 实测中，worker 后续请求中位数比 one-shot 快 2.227 倍，20 次总耗时
减少 53.6%；响应一致、20 次后 RSS 增长 0.719 MiB、崩溃后可恢复。常驻工作集约
521 MiB，属于明确的内存换吞吐。默认保持 worker，同时保留 one-shot 诊断开关与自动
降级；详细环境和原始数据见
