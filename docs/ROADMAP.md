...
- [ ] 数据库完整性预检、一致性备份、恢复前校验、两阶段恢复与失败回滚
- [ ] 迁移前自动备份，以及无法打开主库时仍可使用的最小恢复入口
- [ ] 数据体检与索引维护（`REINDEX` / `ANALYZE`）
- [ ] Windows 安装包验证
- [ ] macOS / Linux 可行性与打包验证
- [ ] 1.0 规模守门：单项目 10,000 张截图、10,000 文件来源目录和 100 个角色的启动、
  查询、监听、缩略图与 Face Bank 性能基线；只修复未达标路径
