...
- “视觉处理参数”配置检测、标注、头像裁剪和主脸框选（扩边重试比例、选区内多脸策略）；
  留空字段沿用 Python 默认值。
...
- `release-windows.ps1` 的 AI sidecar 步骤复用 `%TEMP%\sv-ai-build-venv`。若该目录
  损坏（典型报错 `No pyvenv.cfg file`、pip 退出码 106），直接删除该目录后重跑，脚本会
  重新创建。
- 标签由 `release-windows.ps1 -CreateTag` 在**构建成功之后**创建，所以"本地打包"与
  "GitHub Actions 打包"默认串行。需要并行时先自行 `git tag -a vX.Y.Z` 并推送，脚本
  末尾那步会因标签已存在而中止（安装包在此之前已生成）。
- 本机 Windows 侧没有安装 `git`：`commit.ps1` 的版本提升能正常执行，但 `git add`
  / `git commit` 需要在 WSL 工作区里补跑同样的两条命令。
